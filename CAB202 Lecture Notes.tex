...
\end{minted}
Bitfield order is implementation defined. While bit
\chapter{Interrupts}
...
To configure \mintinline{c}{BUTTON0} as an interrupt source, we must first enable the interrupt in the \mintinline{c}{PORTA.PIN4CTRL} peripheral.
\begin{minted}{c}
ISR(PORTA_PORT_vect) {
...
Compilers can often be configured to produce code in various ways. Compiling without
optimisations will produce code which closely resembles the source code, so that it is easier to debug.
