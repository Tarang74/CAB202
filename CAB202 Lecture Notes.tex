...
\begin{minted}{c}
uint16_t a = 12345;
uint8_t *b = (uint8_t *)&a; // likely contains 57, but may vary on different platforms
\end{minted}
...
Note that the following will not work on the ATtiny1626 because the \mintinline{c}{const} qualifier
...
int a = 5;
\end{minted}
Note that variable must be declared first before it can be used.
...
\begin{itemize}
    ...
    \item peripherals are not subject to timing constrains (execution time of instructions, CPU clock speed)
...
void tca_init() {
    ...
    TCA0.SINGLE.PER = 0xFFFF;                 // Set period to maximum value
    TCA0.SINGLE.CMP1 = 0xFFFE;                // 100% duty cycle
...
using the buffered register \mintinline{c}{TCA0.SINGLE.CMP1BUF}.
...
Analogue to digitalconversion is the process of discretising a continuous signal (typically a voltage)
...
As mentioned in previous sections, a string is a sequence of characters, terminated by a null character (\mintinline{c}{'\0'}).
...
\begin{itemize}
    ...
    \item \mintinline{c}{strchr()} --- Find string in string
...
The \mintinline{c}{putchar(0)} and \mintinline{c}{getchar()} functions are used to write a single character to the standard output device and read a single character from the standard input device, respectively.
...
// If the string is "5", then "5" will stay in the buffer and the function will return 0
...
// If the input is "123", then i will be 123 and the function will return 0
// Otherwise, the function returns 0
\end{minted}
...
ranges can be specified by using a hyphen (\mintinline{c}{-}), and the backslash (\mintinline[escapeinside=||]{c}{|\\|}) can be used to escape special characters.
